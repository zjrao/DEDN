...
# environment 2: 66-74
# material 3: 78-85
# others 4: 75-77, 86-102

indicator = torch.tensor([0] * 102)
indicator[38:65] = 1
indicator[65-74] = 2
indicator[77:85] = 2
indicator[74-77] = indicator[85-102] = 3

#num_cls = torch.tensor([38, 27, 9, 8, 20])
#indexa = torch.tensor([0, 38, 65, 74, 82, 102])

num_cls = torch.tensor([38, 27, 17, 20])
indexa = torch.tensor([0, 38, 65, 82, 102])

class SubNet(nn.Module):
    def __init__(self, init_w2v_att, dim_f, dim_v, dim_r, hidd_f, trainable):
        ...
        #gzsl 8  zsl 16
        self.H_1 = nn.Parameter(nn.init.normal_(torch.empty(dim_v, hidd_f)), requires_grad=True)
        self.H_2 = nn.Parameter(nn.init.zeros_(torch.empty(dim_v, dim_f)), requires_grad=True)
        self.H_3 = nn.Parameter(nn.init.normal_(torch.empty(hidd_f, dim_f)), requires_grad=True)

        self.H_4 = nn.Parameter(nn.init.normal_(torch.empty(dim_v, dim_r*dim_r)), requires_grad=True)
        self.H_5 = nn.Parameter(nn.init.zeros_(torch.empty(dim_v, dim_r*dim_r)), requires_grad=True)

    def forward(self, Fs):
        S_c = torch.einsum('iv,vf,fc,bcr->bir', self.V, self.H_1, self.H_3, Fs)
        A_c = torch.einsum('iv,vf,bfr->bir', self.V, self.H_2, Fs)
        A_c = F.softmax(A_c, dim=-1)
        S_pc = torch.einsum('bir,bir->bir', A_c, S_c)

        Fs = Fs.transpose(1, 2)
        S_c = torch.einsum('iv,vf,bfr->bir', self.V, self.H_4, Fs)
        A_c = torch.einsum('iv,vf,bfr->bir', self.V, self.H_5, Fs)
        A_c = F.softmax(A_c, dim=-1)
...
class DEDN(nn.Module):
    ...
    def __init__(self, dim_f, dim_v, dim_r, hidd_f, init_w2v_att, att, normalize_att,
        ...
        self.GNet = SubNet(init_w2v_att, dim_f, dim_v, dim_r, hidd_f, trainable_w2v)

        self.Snet0 = SubNet(init_w2v_att[indicator == 0], dim_f, dim_v, dim_r, hidd_f, trainable_w2v)
        self.Snet1 = SubNet(init_w2v_att[indicator == 1], dim_f, dim_v, dim_r, hidd_f, trainable_w2v)
        self.Snet2 = SubNet(init_w2v_att[indicator == 2], dim_f, dim_v, dim_r, hidd_f, trainable_w2v)
        self.Snet3 = SubNet(init_w2v_att[indicator == 3], dim_f, dim_v, dim_r, hidd_f, trainable_w2v)
        #self.Snet4 = SubNet(init_w2v_att[indicator == 4], dim_f, dim_v, dim_r, hidd_f, trainable_w2v)

        self.weight_ce = nn.Parameter(torch.eye(self.nclass).float(),
                                      requires_grad=False)  # nn.Parameter(torch.tensor(weight_ce).float(),requires_grad = False)
    ...
    def compute_aug_cross_entropy(self, in_package):
        ...
        if self.is_bias:
            S_pp = S_pp - self.vec_bias  # remove the margin
            if self.mal:
    ...
    def compute_contrastive_loss(self, in_package1, in_package2):
        ...
        closs2 = self.compute_contrastive_loss1(in_package1['S_pp'],
                                                in_package2['S_pp'])
