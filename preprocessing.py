...
def extract_features(config):
    ...
    #ctof_attribute = f'w2v/{config.dataset}_C2F.pkl'
    ...
    with torch.no_grad():
        all_features = []
        for iter, imgs in enumerate(dataset_loader):
            imgs = imgs.to(config.device)
            features = resnet101(imgs)
            all_features.append(features.cpu().numpy())
            if iter%100==0:
                print((iter + 1) * config.batch_size)
    ...
    #with open(ctof_attribute, 'rb') as f:
    #    c2v_att = pickle.load(f)
    ...
    #f.create_dataset('c2v_att', data=c2v_att, compression=compression)
    f.close()
...
if __name__=='__main__':
    ...
    parser.add_argument('--nun_workers', '-n', type=int, default='4')
